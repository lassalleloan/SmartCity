...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`detailsDesEvenements` (
  `idEvenement`       INT,
  `nomEvenement`      INT,
  `nomRubriqueEnfant` INT,
  `nomRubriqueParent` INT,
  `nomRue`            INT,
  `numeroDeRue`       INT,
  `numeroNpa`         INT,
  `latitude`          INT,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`utilisateursDeConfiance` (
  `idRubriqueEnfant`  INT,
  `nomRubriqueEnfant` INT,
  `nomRubriqueParent` INT,
  `idUtilisateur`     INT,
  `personnePhysique`  INT,
  `abreviation`       INT,
  `nomUtilisateur`    INT,
  `prenom`            INT,
  `email`             INT,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`rubriquesEnfantsParents` (
  `idRubriqueEnfant`  INT,
  `nomRubriqueEnfant` INT,
  `nomRubriqueParent` INT
);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`evenementsUtilisateurs` (
  `idEvenement`       INT,
  `nomEvenement`      INT,
  `nomRubriqueEnfant` INT,
  `nomRubriqueParent` INT,
  ...
  `commentaire`       INT,
  `creation`          INT,
  `derniereMiseAJour` INT
);

-- -----------------------------------------------------
-- Placeholder table for view `smartcity`.`detailsUtilisateurs`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`detailsUtilisateurs` (
  `idutilisateur`    INT,
  `AVS`              INT,
  `personnePhysique` INT,
  `abreviation`      INT,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`detailsAdresses` (
  `idAdresse`   INT,
  `nomRue`      INT,
  `numeroDeRue` INT,
  `numeroNpa`   INT
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `smartcity`.`pseudosMotsDePasse` (
  `pseudo`            INT,
  `motDePasse`        INT,
  `sel`               INT,
  `creation`          INT,
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`RubriqueParent_BEFORE_INSERT`
BEFORE INSERT ON `RubriqueParent`
FOR EACH ROW
  BEGIN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`RubriqueParent_BEFORE_UPDATE`
BEFORE UPDATE ON `RubriqueParent`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`RubriqueEnfant_BEFORE_INSERT`
BEFORE INSERT ON `RubriqueEnfant`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`RubriqueEnfant_BEFORE_UPDATE`
BEFORE UPDATE ON `RubriqueEnfant`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Priorite_BEFORE_INSERT`
BEFORE INSERT ON `Priorite`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Priorite_BEFORE_UPDATE`
BEFORE UPDATE ON `Priorite`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Statut_BEFORE_INSERT`
BEFORE INSERT ON `Statut`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Statut_BEFORE_UPDATE`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Rue_BEFORE_INSERT`
BEFORE INSERT ON `Rue`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Rue_BEFORE_UPDATE`
BEFORE UPDATE ON `Rue`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Npa_BEFORE_INSERT`
BEFORE INSERT ON `Npa`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Npa_BEFORE_UPDATE`
BEFORE UPDATE ON `Npa`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Adresse_BEFORE_INSERT`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Adresse_BEFORE_UPDATE`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`TitreCivil_BEFORE_INSERT`
BEFORE INSERT ON `TitreCivil`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`TitreCivil_BEFORE_UPDATE`
BEFORE UPDATE ON `TitreCivil`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Nationalite_BEFORE_INSERT`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Nationalite_BEFORE_UPDATE`
BEFORE UPDATE ON `Nationalite`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Sexe_BEFORE_INSERT`
BEFORE INSERT ON `Sexe`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Sexe_BEFORE_UPDATE`
BEFORE UPDATE ON `Sexe`
FOR EACH ROW
  BEGIN
    IF (NEW.derniereMiseAJour < '1970-01-01 00:00:01' AND NEW.derniereMiseAJour > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Utilisateur_BEFORE_INSERT`
BEFORE INSERT ON `Utilisateur`
FOR EACH ROW
  BEGIN
    IF (NEW.personnePhysique AND
        (NEW.avs IS NULL OR NEW.prenom IS NULL OR NEW.dateDeNaissance IS NULL OR NEW.idSexe IS NULL OR
         NEW.idNationalite IS NULL))
    THEN
      SIGNAL SQLSTATE '44000'
      SET MESSAGE_TEXT = 'personnePhysique est VRAI mais avs, prenom, dateDeNaissance, idSexe ou idNationalite sont NULL';
    ELSEIF (NOT (NEW.personnePhysique) AND
            (NEW.avs IS NOT NULL OR NEW.prenom IS NOT NULL OR NEW.dateDeNaissance IS NOT NULL OR NEW.idSexe IS NOT NULL
             OR NEW.idNationalite IS NOT NULL))
      THEN
        SIGNAL SQLSTATE '44000'
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Utilisateur_BEFORE_UPDATE`
BEFORE UPDATE ON `Utilisateur`
FOR EACH ROW
  BEGIN
    IF (NEW.personnePhysique AND
        (NEW.avs IS NULL OR NEW.prenom IS NULL OR NEW.dateDeNaissance IS NULL OR NEW.idSexe IS NULL))
    THEN
      SIGNAL SQLSTATE '44000'
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Evenement_BEFORE_INSERT`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Evenement_BEFORE_UPDATE`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Commentaire_BEFORE_INSERT`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Commentaire_BEFORE_UPDATE`
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Confiance_BEFORE_INSERT`
BEFORE INSERT ON `Confiance`
FOR EACH ROW
  BEGIN
    IF (NEW.creation < '1970-01-01 00:00:01' AND NEW.creation > '2038-01-19 03:14:07')
    THEN
...
USE `smartcity`$$
CREATE DEFINER = CURRENT_USER TRIGGER `smartcity`.`Confiance_BEFORE_UPDATE`
BEFORE UPDATE ON `Confiance`
FOR EACH ROW
  BEGIN
    IF (NEW.creation <> OLD.creation)
    THEN
